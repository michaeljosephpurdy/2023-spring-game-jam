...
EndButton.new = function(x, y, world)
  ...
  button.untouched_button = love.physics.newBody(world, x + 8, y + 8, 'kinematic') --remember, the shape (the rectangle we create next) anchors to the body from its center, so we have to move it to (650/2, 650-50/2)
  button.untouched_shape = love.physics.newRectangleShape(4, 4)
  button.untouched_fixture = love.physics.newFixture(button.untouched_button, button.untouched_shape, 1) --attach shape to body
  ...
  button.update = function(self)
    ...
    local new_position = self.untouched_button:getPosition()
    if self.old_position then
      print('new')
      print(new_position)
      print('old')
      print(self.old_position)
      if self.old_position ~= new_position then
        self.activated = true
      end
    end
